 ...
 */
async function searchMovieURL(movieTitle, tmdbURL, apiKey) {
...
async function formatData(movieTitles, summaries, sessions, cast, movieLinks) {
    ...
    const output = temp.map((movie, i) => ({ ...movie, bot_text: nowShowingBotText[i] }));
    return output;
};

// async function writeData(data) {
//     const dataToJSON = JSON.stringify(data);
//     await fs.writeFile('../data/data1.json', dataToJSON, (err) => {
//         if (!err) {
//             console.log('done');
//         }
//     });
